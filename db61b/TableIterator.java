...
class TableIterator {
    ...
    Row next() {
        if (_tableIter.hasNext()) {
            _row = _tableIter.next();
            return _row;
        } else {
            _row = null;
            return _row;
    ...
    private Row _row;
    /** The Iterator that helps iterate through the rows of a Table. */
    private Iterator<Row> _tableIter;
